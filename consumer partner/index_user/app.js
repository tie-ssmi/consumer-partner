// app.js - Version: Fixed Timestamp, Rating, Review & Notifications
...
(function() {
    ...
    if (!user && !publicPages.includes(page) && page !== '') window.location.href = 'login.html';
    if (user && publicPages.includes(page)) window.location.href = 'index.html';
})();
...
function injectBottomNav() {
    ...
    const menuItems = [
        ...
        { name: 'Order', link: 'index_order.html', icon: 'fa-file-invoice' },
        { name: 'Task', link: 'work.html', icon: 'fa-list-check' },
        { name: 'Service', link: 'service.html', icon: 'fa-headset' },
        { name: 'My', link: 'profile.html', icon: 'fa-user' }
    ];
...
async function startGrabbing() {
    ...
    try {
        ...
        if(e.message === "PENDING_ORDER") {
            if(confirm("คุณมีออเดอร์ที่ยังไม่เสร็จสิ้น! กรุณาทำรายการให้เสร็จก่อน")) window.location.href = "index_order.html";
...
function logout() { if(confirm("ออกจากระบบ?")) { if(unsubscribeUser) unsubscribeUser(); if(unsubscribeFrozen) unsubscribeFrozen(); localStorage.removeItem('currentUser'); localStorage.removeItem('frozenAmount'); window.location.href = 'login.html'; } }