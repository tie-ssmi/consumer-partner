// app.js - Version: Fixed Path for Root & Subfolder

// --- 0. Path Helper (ฟังก์ชันช่วยตรวจสอบว่าอยู่โฟลเดอร์ไหน) ---
function getPathConfig() {
    const path = window.location.pathname;
    // ตรวจสอบว่าเราอยู่ในโฟลเดอร์ index_user หรือไม่
    const isInSubfolder = path.includes("index_user");
    
    return {
        // ถ้าอยู่ในโฟลเดอร์ ลิ้งก์ Home ต้องถอยกลับ (../) ถ้าอยู่ข้างนอก ก็แค่ index.html
        homeLink: isInSubfolder ? "../index.html" : "index.html",
        // ถ้าอยู่ในโฟลเดอร์ ไม่ต้องเติม prefix ถ้าอยู่ข้างนอก ต้องเติม index_user/
...
(function() {
    ...
    // หน้า Public ที่อนุญาตให้เข้าได้โดยไม่ต้อง Login
    // ถ้าอยู่หน้า Root (index.html) page อาจจะเป็นค่าว่างหรือ index.html
    const isHomePage = page === 'index.html' || page === ''; 

    const user = localStorage.getItem('currentUser');
    
    if (!user) {
        // ถ้าไม่มี User และไม่ใช่หน้า Public -> ดีดไปหน้า Login
        // เช็คว่าหน้าปัจจุบันคือหน้า Public หรือไม่ (ต้องเช็คชื่อไฟล์เพียวๆ)
        if (!publicPages.includes(page) && !isHomePage) {
            // กรณีเปิดไฟล์ html ตรงๆ บางที page อาจไม่ตรง logic นี้ แต่กันเหนียวไว้
             window.location.href = pathConfig.prefix + 'login.html';
        }
        // *เพิ่มเติม: ถ้าอยู่หน้า Home แต่ยังไม่ Login จะให้ดีดไป Login ไหม?
        // ปกติ App แนวนี้ถ้าเข้าหน้า Home แล้วไม่มี User มักจะดีดไป Login ทันที
        if (isHomePage) {
             window.location.href = pathConfig.prefix + 'login.html';
        }
    } else {
        // ถ้ามี User แล้ว แต่พยายามเข้าหน้า Login -> ดีดกลับ Home
...
// ✅ ใช้ var เพื่อให้ index_order.html มองเห็นและส่งข้อมูลกลับมาได้
...
function startSystem() {
    ...
    // ✅ เรียก Heartbeat ตรงนี้ที่เดียวพอ (ปลอดภัยจาก Loop)
    startHeartbeat();
}
...
function startHeartbeat() {
    ...
    updateStatus();
    // อัปเดตทุก 2 นาทีเพื่อประหยัดโควต้า
...
// --- 5. Navbar (Fixed for Root/Subfolder) ---
function injectBottomNav() {
    const navContainer = document.getElementById('bottom-nav');
    if (!navContainer) return;

    // เรียกใช้ Config Path ที่เราทำไว้
    const pathConfig = getPathConfig();
    ...
    menuItems.forEach(item => {
        // เช็ค Active ด้วย ID แทน Link เพราะ Link เปลี่ยนไปตาม Path
...
function injectUniversalModals() {
    ...
    <div id="modal-withdraw-step2" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-[80] p-4 font-sans backdrop-blur-sm" onclick="if(event.target===this) hideModal('modal-withdraw-step2')"><div class="bg-white rounded-xl w-full max-w-sm p-5 relative animate-[scaleIn_0.2s_ease-out]"><button onclick="hideModal('modal-withdraw-step2')" class="absolute top-2 right-2 text-gray-400"><i class="fa-solid fa-xmark text-lg"></i></button><h3 class="font-bold text-center text-lg mb-2">รายละเอียดการถอน</h3><div class="bg-gray-50 p-3 rounded-lg text-xs space-y-2 mb-4 border border-gray-100"><div class="flex justify-between"><span>ยอดเงินสุทธิ (Net):</span> <span id="wd-total" class="font-bold text-black">...</span></div><div class="flex justify-between text-red-500"><span>ถูกแช่แข็ง (Frozen):</span> <span id="wd-frozen-show">...</span></div><div class="flex justify-between text-gray-500"><span>ค่าธรรมเนียม (<span id="wd-fee-rate">5</span>%):</span> <span id="wd-fee-amt">...</span></div><div class="border-t border-dashed border-gray-300 my-1"></div><div class="flex justify-between text-base font-bold text-[#DC2626]"><span>ถอนได้จริง:</span> <span id="wd-available">...</span></div></div><div class="relative mb-2"><input type="number" id="withdraw-amount" class="w-full border-2 border-red-100 rounded-lg p-3 pr-16 text-xl font-bold text-[#DC2626] outline-none focus:border-[#DC2626] transition" placeholder="0.00" oninput="calculateRealTimeFee(this.value)"><button onclick="fillMaxWithdraw()" class="absolute right-2 top-2 bottom-2 px-3 bg-red-100 text-[#DC2626] text-xs font-bold rounded hover:bg-red-200">MAX</button></div><p class="text-[10px] text-right text-gray-400 mb-3" id="wd-fee-preview">ค่าธรรมเนียม: ฿0.00</p><input type="password" id="withdraw-password" class="w-full border border-gray-200 rounded-lg p-2 text-center text-sm mb-4" placeholder="รหัสผ่านยืนยัน"><button onclick="confirmWithdrawal()" class="w-full bg-[#DC2626] text-white py-3 rounded-lg font-bold shadow-md hover:bg-red-700 active:scale-95 transition text-sm">ยืนยันการถอน</button></div></div>
...
// --- Notifications Logic (Your Custom Code Preserved) ---
    ...
    try {
        ...
        await withTimeout(Promise.all([
            window.db.collection("orders").doc(order.docId).update({ 
                ...
                // ✅ อัปเดต timestamp เป็นปัจจุบันเพื่อให้เด้งไปบนสุด
                timestamp: new Date().toISOString(), 
...
function calculateRealTimeFee(val) { const amount = parseFloat(val); if (!amount || amount <= 0) { document.getElementById('wd-fee-preview').innerText = 'ค่าธรรมเนียม: ฿0.00'; setTxt('wd-fee-amt', '฿0.00'); return; } const fee = (amount * withdrawInfo.feeRate) / 100; setTxt('wd-fee-amt', `฿${fee.toLocaleString()}`); document.getElementById('wd-fee-preview').innerText = `ค่าธรรมเนียม: ฿${fee.toLocaleString()} (รับจริง: ฿${(amount - fee).toLocaleString()})`; }
...
function logout() { 
    if(confirm("ออกจากระบบ?")) { 
        ...
        // แก้ไข: ใช้ Path Config เพื่อดีดไปหน้า Login ให้ถูกที่
        const pathConfig = getPathConfig();
        window.location.href = pathConfig.prefix + 'login.html'; 
    } 
}
